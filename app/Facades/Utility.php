...
class Utility
{
        ...
        if (count($values) > 0) {
            foreach ($values as $envKey => $envValue) {
                ...
                if ($keyPosition === false) {
                    ...
                    $currentValue = trim(explode('=', $oldLine)[1], "'");
    ...
    {
        $chartArray = [];
        $form_values = FormValue::select('forms.json as form_json', 'form_values.*')->join('forms', 'forms.id', '=', 'form_values.form_id')->where('form_id', $form_id)
        ->when(!\Auth::user()->can('access-all-submitted-form'), function($query) {

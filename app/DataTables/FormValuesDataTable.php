...
class FormValuesDataTable extends DataTable
{
    public function dataTable($query)
    {
        $data = datatables()
            ...
            ->editColumn('payment_type', function (FormValue $formValue) {
                if ($formValue->payment_type == "offlinepayment") {
                    $paymentType = '<a href="' . route('payment.slip.download', $formValue->id) . '" class="p-2 px-3 text-white badge bg-primary rounded-pill">' . __('Offline Payment') . '</a>';
                    return $paymentType;
                } else {
            ...
            ->addColumn('form_status', function (FormValue $formValue) {
                ...
                if ($formStatus != null) {
                    ...
                    $label = '<span class="badge bg-light-' . $color . ' p-2 rounded">' . $name . '</span>';
                } else {
        ...
        $labels = [];
        $labels = $this->labels();
        if ($labels && !empty($labels) && $labels != null) {
            ...
            $arr = array_merge(['status', 'action', 'user', 'type', 'created_at', 'payment_type', 'form_status'], array_keys($labels));
        } else {
    ...
    public function labels()
    {
        ...
        if ($formValue && $formValue->json != null) {
            $jsonData = $formValue->json;
            $jsonArray = json_decode($jsonData, true);
            $filteredData = [];
    ...
    public function html()
    {
        $dataTable = $this->builder()
            ...
            ->ajax([
                'url' => route('get.forms',$this->form_id),
                'type' => 'GET',
                'data' => 'function(d) {

...
class RcmsDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('clasificacion', function (Rcm $rcm) {
                if ($rcm->clasificacion == 1) {
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-primary">' . __('RCM') . '</span>';
                    return $out;
                } else if ($rcm->clasificacion == 0){
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-warning">' . __('Ventanilla') . '</span>';
                    return $out;
                } else if ($rcm->clasificacion == 2){
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-secondary">' . __('Kioscos') . '</span>';
                    return $out;
                } else {
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-danger">' . __('Bodega') . '</span>';
                    return $out;
                }
            })
            ...
            ->editColumn('nombre_local', function (Rcm $rcm) {
                return $rcm->Contrato ? strtoupper($rcm->Contrato->nombre_local) : "";
            })
            ...
            ->rawColumns(['action', 'status', 'clasificacion']);
    }

    public function query(Rcm $model, Request $request)

    {
        $query =  $model->newQuery()->with(['Contrato'])->select(['rcms.*', 'deptos.nombredepto as nombredepto', 'municipios.nombremunicipio as nombremunicipio', 'users.name as username'])
...
",
    ...
    protected function getColumns(): array
    {
        return [
            ...
            Column::make('nombre_local')->title(__('Local')),
            Column::make('clasificacion')->title(__('Tipo')),

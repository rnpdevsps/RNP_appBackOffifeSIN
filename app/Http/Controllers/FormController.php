...
class FormController extends Controller
{
    ...
    public function destroy(Form $form)
    {
        if (Auth::user()->can('delete-form')) {
            ...
            foreach ($feedbacks as $feedback) {
                $feed = Feedback::find($feedback->id);
                if ($feed) {
                    $feed->delete();
                }
            }
    ...
    public function fillStore(Request $request, $id)
    {
        ...
        if ($form) {
            ...
            if (UtilityFacades::getsettings('email_setting_enable') == 'on') {
                $ccemails = array_filter(array_map('trim', explode(',', $form->ccemail)));
                $bccemails = array_filter(array_map('trim', explode(',', $form->bccemail)));
                try {
                    $mail = Mail::to($form->email);
                    ...
                    $mail->send(new FormSubmitEmail($formValue, $formValueArray));
                } catch (\Exception $e) {
                    if ($request->ajax()) {
                        return response()->json(['is_success' => false, 'message' => $e->getMessage()], 200);
                    } else {
            ...
            if (isset($notificationsSetting)) {
                ...
                if ($notificationsSetting->email_notification == '1' && !empty($formValue->email)) {
                    if (UtilityFacades::getsettings('email_setting_enable') == 'on') {
                        try {
            ...
            if (isset($request->ajax)) {
                return response()->json(['is_success' => true, 'message' => $successMsg, 'formValueId' => $formValue->id, 'redirect' => route('edit.form.values', $formValue->id)], 200);
            } else {
    ...
    public function destroyMultiple(Request $request)
    {
        if (Auth::user()->can('delete-form')) {
            ...
            $feedback = Feedback::whereIn('form_id', $request->ids)->delete();
    ...
    public function restore($id)
    {
        ...
        $feedback = Feedback::where('form_id', $id)->restore();
    ...
    public function restoreMultiple(Request $request)
    {
        ...
        $feedback = Feedback::whereIn('form_id', $request->ids)->restore();
    ...
    public function forcedelete($id)
    {
        ...
        Feedback::where('form_id', $id)->forcedelete();
    ...
    public function forcedeleteMultiple(Request $request)
    {
        ...
        $feedback = Feedback::whereIn('form_id', $request->ids)->forcedelete();
    ...
    public function forcedeleteAll(Request $request)
    {
        ...
        $feedback = Feedback::onlyTrashed()->forcedelete();
    ...
    public function removeUpoadedFile($img, $formId)
    {
        ...
        if (!$formValue) {
            return response()->json(['is_success' => false, 'message' => 'Form value not found'], 404);
        }
        ...
        return response()->json(['is_success' => false, 'message' => 'File not found in form values'], 404);
    }
}

    ...
    <div class="col-md-12">
        ...
        <ul class="breadcrumb">
            <li class="breadcrumb-item">
                {!! html()->a(route('home'), __('Dashboard')) !!}
            </li>
            <li class="breadcrumb-item">
                {!! html()->a(route('forms.index'), __('Forms')) !!}
            </li>
...
@push('script')
    ...
    @if ($form->payment_status == 1)
        ...
        @if (Utility::getsettings('PAYPAL_SANDBOX_CLIENT_ID'))
            <script src="https://www.paypal.com/sdk/js?client-id={{ Utility::getsettings('PAYPAL_SANDBOX_CLIENT_ID') }}&currency={{ $booking->currency_name }}"></script>
        @endif
    ...
    <script>
        ...
        function submitForm(formData) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let errorMessage = xhr.status === 422 ? Object.values(xhr.responseJSON.errors).map(val => val[0]).join('<br>') : xhr.responseJSON.message;
                    show_toastr(xhr.status === 422 ? 'Validation Error!' : 'Error!', errorMessage, 'danger');
                    $('#nextBtn').removeAttr('disabled').html('Submit');
                    showTab($('.tab').length - 1);
                }
            });
                ...
                @if ($form->payment_status == 1)
                    ...
                    @elseif ($form->payment_type == 'paystack')
                        ...
                        var handler = PaystackPop.setup({
                            ...
                            onClose: function() {
                                show_toastr('Failed!', 'Transaction was not completed, window closed.','danger');
                            },
                        });
                        handler.openIframe();

            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Fast2SMS Number'), 'fast2sms_number' . $fast2smsJsonkey)->class('form-label') !!}
                        {!! html()->number('fast2sms_number[]', isset($fast2smsJson['fast2sms_number']) ? $fast2smsJson['fast2sms_number'] : null)->class('form-control')->id('fast2sms_number' . $fast2smsJsonkey)->required()->placeholder(__('Enter fast2sms number')) !!}
                        <small>{{ __('Note: Please enter the mobile number for send sms. Example: 1234567890') }}</small>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Fast2SMS Api Key'), 'fast2sms_api_key' . $fast2smsJsonkey)->class('form-label') !!}
                        {!! html()->text('fast2sms_api_key[]', isset($fast2smsJson['fast2sms_api_key']) ? $fast2smsJson['fast2sms_api_key'] : null)->class('form-control')->id('fast2sms_api_key' . $fast2smsJsonkey)->required()->placeholder(__('Enter fast2sms api key')) !!}
                    </div>

                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Fast2SMS Field'), 'fast2sms_field' . $fast2smsJsonkey)->class('form-label') !!}

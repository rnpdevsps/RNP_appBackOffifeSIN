            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! Html::label(__('Mailgun Email'), 'mailgun_email' . $mailgunJsonkey)->class('form-label') !!}
                        {!! Html::email('mailgun_email[]', isset($mailgunJson['mailgun_email']) ? $mailgunJson['mailgun_email'] : null)->class('form-control')->id('mailgun_email' . $mailgunJsonkey)->required()->placeholder(__('Enter mailgun email')) !!}
                        <small>{{ __("Note: Please enter the email address to be used as the sender's email for Mailgun.") }}</small>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! Html::label(__('Mailgun Domain'), 'mailgun_domain' . $mailgunJsonkey)->class('form-label') !!}
                        {!! Html::text('mailgun_domain[]', isset($mailgunJson['mailgun_domain']) ? $mailgunJson['mailgun_domain'] : null)->class('form-control')->id('mailgun_domain' . $mailgunJsonkey)->required()->placeholder(__('Enter mailgun domain')) !!}
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! Html::label(__('Mailgun Secret'), 'mailgun_secret' . $mailgunJsonkey)->class('form-label') !!}
                        {!! Html::text('mailgun_secret[]', isset($mailgunJson['mailgun_secret']) ? $mailgunJson['mailgun_secret'] : null)->class('form-control')->id('mailgun_secret' . $mailgunJsonkey)->required()->placeholder(__('Enter mailgun secret')) !!}

                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! Html::label(__('Mailgun Field'), 'mailgun_field' . $mailgunJsonkey)->class('form-label') !!}

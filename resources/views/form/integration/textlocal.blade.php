            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Textlocal Number'), 'textlocal_number' . $textlocalJsonkey)->class('form-label') !!}
                        {!! html()->number(
                                'textlocal_number[]',
                                isset($textlocalJson['textlocal_number']) ? $textlocalJson['textlocal_number'] : null,
                            )->class('form-control')->id('textlocal_number' . $textlocalJsonkey)->required()->placeholder(__('Enter textlocal number')) !!}
                        <small>{{ __('Note: Please enter the mobile number for send sms. Example: 911234567890') }}</small>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Textlocal Api Key'), 'textlocal_api_key' . $textlocalJsonkey)->class('form-label') !!}
                        {!! html()->text(
                                'textlocal_api_key[]',
                                isset($textlocalJson['textlocal_api_key']) ? $textlocalJson['textlocal_api_key'] : null,
                            )->class('form-control')->id('textlocal_api_key' . $textlocalJsonkey)->required()->placeholder(__('Enter textlocal api key')) !!}
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="form-group">
                        {!! html()->label(__('Textlocal Field'), 'textlocal_field' . $textlocalJsonkey)->class('form-label') !!}

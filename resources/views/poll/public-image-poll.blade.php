...
@section('content')
    <div class="row">
        {!! html()->modelForm($poll, 'POST', route('image.poll.store', $poll->id))->attribute('data-validate')->class('form-horizontal')->open() !!}
        <div class="row">
                        ...
                        <div class="text-end">
                            ...
                            @elseif(
                                $poll->image_results_visibility == 'public_after_end_date' &&
                                    Carbon::parse($poll->image_set_end_date_time)->lessThanOrEqualTo(Carbon::now()))
                                <a href="{{ route('poll.public.result.image', $id) }}" class="btn btn-success"><i
                                        class="me-2" data-feather="award"></i>{{ __('Result') }}</a>
                            @elseif(
                                $poll->image_results_visibility == 'public_after_vote' &&
                                    ImagePoll::where('poll_id', $poll->id)->where('session_id', $newSessid)->first() != null)
                                <a href="{{ route('poll.public.result.image', $id) }}" class="btn btn-success"><i
                                        class="me-2" data-feather="award"></i>{{ __('Result') }}</a>
                            @else
                            @endif

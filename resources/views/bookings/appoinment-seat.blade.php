...
@section('title', __('Appoinment Seat Wise'))
@section('breadcrumb')
    <div class="col-md-12">
        <div class="page-header-title">
            <h4 class="m-b-10">{{ __('Appoinment Seat Wise') }}</h4>
        </div>
        <ul class="breadcrumb">
            <li class="breadcrumb-item">{!! Html::a()->href(route('home'))->text(__('Dashboard')) !!}</li>
            <li class="breadcrumb-item">{!! Html::a()->href(route('bookings.index'))->text(__('Booking')) !!}</li>
            <li class="breadcrumb-item">{{ __('Appoinment Seat Wise') }}</li>
        </ul>
    </div>
@endsection
@push('style')
...
@push('script')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            bookingStartDate.setHours('{{ $currDate->format('H') }}', '{{ $currDate->format('i') }}', 0, 0);
            bookingEndDate.setHours('{{ $currDate->format('H') }}', '{{ $currDate->format('i') }}', 0, 0);
            currentDate.setHours('{{ $currDate->format('H') }}', '{{ $currDate->format('i') }}', 0, 0);

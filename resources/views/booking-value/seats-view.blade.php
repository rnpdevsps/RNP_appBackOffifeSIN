...
@section('breadcrumb')
    <div class="col-md-12">
        ...
        <ul class="breadcrumb">
            <li class="breadcrumb-item">{!! html()->a(route('home'))->text(__('Dashboard'))->class('') !!}</li>
            <li class="breadcrumb-item">
                {!! html()->a(route('view.booking.values', $bookingValue->booking_id))->text(__('Submitted Bookings of ' . ' ' . $booking->business_name)) !!}
            </li>
                                ...
                                @foreach ($array as $keys => $rows)
                                    @foreach ($rows as $row_key => $row)
                                        @if ($row->type == 'checkbox-group')
                                            <div class="col-12">
                                                <div class="form-group">
                                                    {!! html()->label($row->label)->for($row->name)->class('form-label') !!}
                                                    @if ($row->required)
                                                    ...
                                                    <p>
                                                        @if (property_exists($row, 'value'))
                                                            @if ($row->value)
                                                                ...
                                                                @if ($row->multiple)
                                                                    <div class="row">
                                                                        @if (Utility::getsettings('storage_type') == 'local')
                                        ...
                                        @elseif($row->type == 'location')
                                            <div class="col-12">
                                                <div class="form-group">
                                                    {!! html()->label('Location:')->for('location_id') !!}
                                        ...
                                        @elseif($row->type == 'video')
                                            @if ($row->value && Storage::exists($row->value))
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        {!! html()->label($row->label)->for($row->name)->class('form-label') !!}<br>
